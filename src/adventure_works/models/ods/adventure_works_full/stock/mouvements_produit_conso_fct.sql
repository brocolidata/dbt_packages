...
mouvements as (
    select
        ...
        sum(IFNULL(cs.stock_initial,0)) as stock_initial,
...
quantite_rest as (
    ...
    select 
        ...
        mv.stock_initial + mv.quantite_achetee - mv.quantite_consommee - mv.quantite_rejetee - mv.quantite_vendue + mv.quantite_produite as quantite_restante
...
quantit_rest_avec_inv as(
    ...
    select 
        qr.* except(stock_initial, quantite_achetee, quantite_consommee, quantite_produite, quantite_rejetee, quantite_vendue, quantite_restante, quantite_sortie, quantite_entree),
        ...
        SUM(qr.quantite_restante) OVER (PARTITION BY id_produit ORDER BY date_mouvement) AS running_quantite_restante
...
avec_inv as(
    ...
    select 
        q.* except(stock_initial, date_mouvement, quantite_restante, quantite_sortie, quantite_entree),
        case 
            when q.running_quantite_restante = min(q.running_quantite_restante) OVER(PARTITION BY id_produit)  and  min(q.running_quantite_restante) OVER(PARTITION BY id_produit) < 0  
        ...
        case 
            when q.running_quantite_restante = min(q.running_quantite_restante) OVER(PARTITION BY id_produit)  and  q.running_quantite_restante < 0  
...
etat as(
    ...
    select 
        ...
        sum(e.stock_initial + e.quantite_entree - e.quantite_sortie) as quantite_restante
...
etat_final as(
    ...
    select 
        ...
        SUM(ef.quantite_restante) OVER (PARTITION BY id_produit ORDER BY date_mouvement) AS running_quantite_restante
